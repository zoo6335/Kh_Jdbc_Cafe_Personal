...
public class SalesDAO {
    public List<SalesVO> salesList() {
        ...
        try{
            ...
            String sql = "SELECT ORD.ORDER_NO, ORD.ORDER_DATE, MENU.MNAME, MENU.MPRICE, " +
                    "OPT.ONAME, OPT.OPRICE, (MENU.MPRICE + OPT.OPRICE) " +
            ...
            while(rs.next()) {
                int ordnum = rs.getInt("ORD.ORDER_NO");      // 주문번호
                Date orddate = rs.getDate("ORD.ORDER_DATE"); // 주문일
                String mname = rs.getString("MENU.MNAME");   // 메뉴명
                int mprice = rs.getInt("MENU.MPRICE");       // 가격
                String oname = rs.getString("OPT.ONAME");    // 추가옵션
                int oprice = rs.getInt("OPT.OPRICE");       //추가금액
                int salesTotal = rs.getInt("MENU.MPRICE + OPT.OPRICE"); // 총 주문금액
